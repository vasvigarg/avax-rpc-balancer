...
export async function proxyRequest(
  ...
  if (isReadOperation && !opts.skipCache) {
    ...
    if (cachedResponse) {
      ...
      if (isBatch) {
        log.info(
          `Serving batch request with ${
            (requestPayload as RpcRequest[]).length
          } operations from cache`,
        );
      } else {
        log.info(
          `Serving request ID ${(requestPayload as RpcRequest).id} (${
            (requestPayload as RpcRequest).method
          }) from cache`,
        );
      }
  ...
  if (isBatch) {
    log.info(
      `Proxying batch request with ${requestPayload.length} operations to ${targetUrl}${
        nodeId ? ` (Node: ${nodeId})` : ''
      }`,
    );
  } else {
    log.info(
      `Proxying request ID ${requestPayload.id} (${requestPayload.method}) to ${targetUrl}${
        nodeId ? ` (Node: ${nodeId})` : ''
      }`,
    );
  }
...
async function executeWithRetry(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      log.error(
        `Attempt ${attempt + 1}/${retries + 1} failed for request ID ${requestId}: ${
          axiosError.message
        }`,
      );
      ...
      if (isLastAttempt) {
        ...
        return createErrorResponse(
          isBatch
            ? (requestPayload as RpcRequest[]).map(r => r.id)
            : (requestPayload as RpcRequest).id,
