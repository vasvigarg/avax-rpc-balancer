...
export interface NetworkManagerEventEmitter extends EventEmitter {
  ...
  emit<E extends keyof NetworkManagerEvents>(
    event: E,
...
export async function getBalance(
  ...
  const balanceHex = await executeRpcCall<string>(networkType, 'eth_getBalance', [
    address,
    blockIdParam,
  ]);
...
export async function getTransactionCount(
  ...
  const countHex = await executeRpcCall<string>(networkType, 'eth_getTransactionCount', [
    address,
    blockIdParam,
  ]);
 ...
 */
export async function isContract(networkType: NetworkType, address: string): Promise<boolean> {
