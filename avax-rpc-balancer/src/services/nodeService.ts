...
export interface NodeServiceEventEmitter extends EventEmitter {
  ...
  emit<E extends keyof NodeServiceEvents>(event: E, ...args: Parameters<NodeServiceEvents[E]>): boolean;
}
...
export async function initializeNodesForNetwork(networkType: NetworkType): Promise<boolean> {
  try {
    ...
    events.emit('network:error', networkType, error instanceof Error ? error : new Error(String(error)));
    return false;
  }
}
...
export async function initializeNode(node: NodeInfo): Promise<boolean> {
  try {
    ...
    const config = {
      ...
      // No auth since it's not in your NodeInfo
    };
...
export function getBestNodeAdapter(networkType: NetworkType): BaseNodeAdapter | undefined {
  ...
  if (anyNode) {
    log.warn(`No healthy nodes available for ${networkType}, using potentially unhealthy node ${anyNode.id}`);
    ...
    for (const altAdapter of adapters) {
      ...
      try {
        ...
        if (altNodeId) {
          ...
          log.info(`Updated preferred node for ${networkType} to ${altNodeId} after successful failover`);
        }
